...
class Instruction {
  public:
    ...
    void setInstr(std::uint32_t p_instr) { m_instr = p_instr; }

    /**
     * @brief return instruction
     * @return all instruction bits (31:0)
     */
    std::uint32_t getInstr() const { return m_instr; }

    inline void dump() const { std::cout << std::hex << "0x" << m_instr << std::dec << std::endl; }
