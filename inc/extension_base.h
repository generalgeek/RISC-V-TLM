...
template <typename T> class extension_base {

    using signed_T = typename std::make_signed<T>::type;
    using unsigned_T = typename std::make_unsigned<T>::type;

  public:
    extension_base(const T& instr, Registers<T>* register_bank, MemoryInterface* mem_interface) :
    ...
    void setInstr(std::uint32_t p_instr) { m_instr = sc_dt::sc_uint<32>(p_instr); }
    ...
    void RaiseException(Exception_cause cause, std::uint32_t inst) {
        ...
        logger->debug("{} ns. PC: 0x{:x}. Exception! new PC 0x{:x} ", sc_core::sc_time_stamp().value(), current_pc,
                      new_pc);
    }
    ...
    virtual unsigned int get_rd() const { return m_instr.range(11, 7); }

    virtual void set_rd(unsigned int value) { m_instr.range(11, 7) = value; }

    virtual unsigned int get_rs1() const { return m_instr.range(19, 15); }

    virtual void set_rs1(unsigned int value) { m_instr.range(19, 15) = value; }

    virtual unsigned int get_rs2() const { return m_instr.range(24, 20); }

    virtual void set_rs2(unsigned int value) { m_instr.range(24, 20) = value; }

    virtual unsigned int get_funct3() const { return m_instr.range(14, 12); }

    virtual void set_funct3(unsigned int value) { m_instr.range(14, 12) = value; }

    // For RV64 SLLI, SRLI & SRAI
    virtual unsigned int get_shamt_slli() const { return m_instr.range(25, 20); }
    virtual void dump() const { std::cout << std::hex << "0x" << m_instr << std::dec << std::endl; }
