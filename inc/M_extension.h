 ...
 */
template <typename T> class M_extension : public extension_base<T> {
  public:
    /**
    ...
    inline void dump() const override { std::cout << std::hex << "0x" << this->m_instr << std::dec << std::endl; }
    ...
    void Exec_M_MUL() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.MUL: x{:d}({:d}) * x{:d}({:d}) -> x{:d}({:d})",
                            sc_core::sc_time_stamp().value(), this->regs->getPC(), rs1, rs2, multiplier, multiplicand,
                            rd, result);
    }
    ...
    void Exec_M_DIV() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.DIV: x{:d} / x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                            this->regs->getPC(), rs1, rs2, rd, result);
    }
    ...
    void Exec_M_DIVU() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.DIVU: x{:d} / x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                            this->regs->getPC(), rs1, rs2, rd, result);
    }
    ...
    void Exec_M_REM() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.REM: x{:d} % x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                            this->regs->getPC(), rs1, rs2, rd, result);
    }
    ...
    void Exec_M_REMU() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.REMU: x{:d} % x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                            this->regs->getPC(), rs1, rs2, rd, result);
    }
    ...
    void Exec_M_DIVW() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.DIVW: x{:d} / x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                            this->regs->getPC(), rs1, rs2, rd, result);
    }
    ...
    void Exec_M_DIVUW() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.DIVUW: x{:d} / x{:d} -> x{:d}({:d})",
                            sc_core::sc_time_stamp().value(), this->regs->getPC(), rs1, rs2, rd, result);
    }
    ...
    void Exec_M_REMW() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.REMW: x{:d} % x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                            this->regs->getPC(), rs1, rs2, rd, result);
    }
    ...
    void Exec_M_REMUW() const {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. M.REMUW: x{:d}({:d}) % x{:d}({:d}) -> x{:d}({:d})",
                            sc_core::sc_time_stamp().value(), this->regs->getPC(), rs1, dividend, rs2, divisor, rd,
                            result);
    }
