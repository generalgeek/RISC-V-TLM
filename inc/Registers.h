...
template <typename T> class Registers {
  public:
     ...
     * @return PC value
     */
    T getPC() const { return register_PC; }

    /**
     * Sets arbitrary value to PC
     * @param new_pc new address to PC
     */
    void setPC(T new_pc) { register_PC = new_pc; }

    /**
     * Increments PC counter to next address
     */
    inline void incPC() { register_PC += 4; }

    inline void incPCby2() { register_PC += 2; }
    ...
    T getCSR(int csr) {
        ...
        switch (csr) {
            case CSR_CYCLE:
            case CSR_MCYCLE:
                ret_value = static_cast<std::uint64_t>(
                                sc_core::sc_time(sc_core::sc_time_stamp() - sc_core::sc_time(sc_core::SC_ZERO_TIME))
                                    .to_double()) &
                            0x00000000FFFFFFFF;
                break;
            case CSR_CYCLEH:
            case CSR_MCYCLEH:
                ret_value = static_cast<std::uint32_t>(
                    (std::uint64_t)(sc_core::sc_time(sc_core::sc_time_stamp() - sc_core::sc_time(sc_core::SC_ZERO_TIME))
                                        .to_double()) >>
                        32 &
                    0x00000000FFFFFFFF);
                break;
