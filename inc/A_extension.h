 ...
 */
template <typename T> class A_extension : public extension_base<T> {
  public:
    /**
     ...
     */
    inline unsigned_T opcode() const override { return static_cast<unsigned_T>(this->m_instr.range(31, 27)); }
    ...
    bool Exec_A_LR() {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. A.LR.W: x{:d}(0x{:x}) -> x{:d}(0x{:x}) ",
                            sc_core::sc_time_stamp().value(), this->regs->getPC(), rs1, mem_addr, rd, data);
    ...
    bool Exec_A_SC() {
        ...
        this->logger->debug("{} ns. PC: 0x{:x}. A.SC.W: (0x{:x}) <- x{:d}(0x{:x}) ", sc_core::sc_time_stamp().value(),
                            this->regs->getPC(), mem_addr, rs2, data);
    ...
    void TLB_reserve(std::uint32_t address) { TLB_A_Entries.insert(address); }
