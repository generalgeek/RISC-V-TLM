...
namespace riscv_tlm {
// RV32
template <> void M_extension<std::uint32_t>::Exec_M_MULH() const {
    ...
    this->logger->debug("{} ns. PC: 0x{:x}. M.MULH: x{:d}({:d}) * x{:d}({:d}) -> x{:d}({:d})",
                        sc_core::sc_time_stamp().value(), this->regs->getPC(), rs1, multiplier, rs2, multiplicand, rd,
                        result);
}

template <> void M_extension<std::uint32_t>::Exec_M_MULHSU() const {
    unsigned int rd, rs1, rs2;
    ...
    this->logger->debug("{} ns. PC: 0x{:x}. M.MULHSU: x{:d} * x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                        this->regs->getPC(), rs1, rs2, rd, result);
}

template <> void M_extension<std::uint32_t>::Exec_M_MULHU() const {
    unsigned int rd, rs1, rs2;
    ...
    this->logger->debug("{} ns. PC: 0x{:x}. M.MULHU: x{:d} * x{:d} -> x{:d}({:d})", sc_core::sc_time_stamp().value(),
                        this->regs->getPC(), rs1, rs2, rd, result);
}

// RV64
// I need to use SystemC bigint with 128 bits to perform 64 x 64 bits multiplication and keep the high half
template <> void M_extension<std::uint64_t>::Exec_M_MULH() const {
    unsigned int rd, rs1, rs2;
    signed_T multiplier, multiplicand;
    signed_T result;
...
template <> void M_extension<std::uint64_t>::Exec_M_MULHSU() const {
    unsigned int rd, rs1, rs2;
    signed_T multiplier;
...
template <> void M_extension<std::uint64_t>::Exec_M_MULHU() const {
    unsigned int rd, rs1, rs2;
